...
public class Reporter : MonoBehaviour
{
	...
	void Start()
	{
        ...
        InvokeRepeating("SaveLogFile", 60, AppConfig.ERROR_LOG_BACKUP);
	}
	...
	void CaptureLogThread(string condition, string stacktrace, LogType type)
	{
        ...
        {
            errorBuilder.AppendLine(DateTime.UtcNow.ToString())
    ...
    private void SaveLogFile()
    {
        ...
        TimeSpan span = DateTime.UtcNow - info.CreationTimeUtc;
        if (Mathf.Abs(span.Days) >= 1)
        {
            File.Delete(path);
        }
        ...
        {
            mLogWriter = new StreamWriter(path);
        }
        mLogWriter.Write(errorBuilder.ToString());
        errorBuilder.Length = 0;
        errorBuilder.Capacity = 0;
        mLogWriter.AutoFlush = true;
        mLogWriter.Close();
        Debug.Log("SaveLogFile");
    }
